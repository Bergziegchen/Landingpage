...
menuButton.addEventListener('click', () => {
    ...
    if (navbar.classList.contains('open')) {
        ...
        // Fokus bewusst auf erstes Element im Menü setzen
        const firstFocusable = navContent.querySelector(
            'a, button, input, textarea, select, [tabindex]:not([tabindex="-1"])'
        );
        if (firstFocusable) firstFocusable.focus({ preventScroll: true });

        // **Warte auf das Ende der Öffnungs-Animation**
        navbar.addEventListener('animationend', function handler(e) {
            if (e.animationName === 'openNav') {
                setDiameter(); // Slider Größe setzen
                navbar.removeEventListener('animationend', handler);
            }
        });
    }

    // aria-expanded immer aktuell halten
...
/*---------------------
Schließen bei Klick auf Link oder Overlay
----------------------*/
...
function updateNavbarSize() {
    if (window.innerWidth > 767) {
        ...
        const baseWidth = currentWidth * 0.4;
        const additional = currentWidth * 0.2;
        const newWidth = baseWidth + additional;
...
window.addEventListener('resize', () => {
    ...
    resizeTimeout = setTimeout(updateNavbarSize, 200);
});

/*---------------------
Tastatursteuerung Menu
----------------------*/
...
function updateSlider2(direction2) {
    ...
    if (direction2 === 'prev' && startOffset2 > 0) {
        startOffset2--;
    }

    cards2.forEach((card2, index2) => {
        const relativeIndex2 = index2 - startOffset2;
        const distance2 = Math.abs(relativeIndex2);

        card2.style.setProperty('--i2', relativeIndex2);
        card2.style.setProperty('--dist2', distance2);

        const isActive2 = relativeIndex2 === 0;
        card2.classList.toggle('active2', isActive2);

        card2.setAttribute('aria-hidden', !isActive2);
        card2.setAttribute('tabindex', isActive2 ? '0' : '-1');
    });
...
function updateButtons2() {
    prevBtn2.disabled = startOffset2 === 0;
    nextBtn2.disabled = startOffset2 === cards2.length - 1;
}
...
document.addEventListener('keydown', (event2) => {
    if (event2.key === 'ArrowLeft') updateSlider2('prev');
    if (event2.key === 'ArrowRight') updateSlider2('next');
});
